...
#TODO: Go through these and decide if they still apply for Ocelot
    ...
    def calc_resyncbaudtrans_reg(self, model):
        ...
        #Based on Series 1 findings, always set RESYNCBAUDTRANS
    ...
    def calc_isicomp_reg(self, model):
        # This function calculate ISICOMP register
        # TODO: figure out how to calculate ISICOMP for 4-FSK
        modulation = model.vars.modulation_type.value

        if modulation == model.vars.modulation_type.var_enum.FSK4:
            self._reg_write(model.vars.MODEM_CTRL4_ISICOMP, 10)
        else:
    ...
    def calc_phscale_reg(self, model):
        ...
        if remoden:
            ...
            if demod_sel == model.vars.demod_select.var_enum.BCR:
                ...
                phscale_reg = int(floor(log(2 * mi, 2)))
    ...
    def calc_ksi1_reg(self, model):
        ...
        if ((trecs_enabled or demod_sel == model.vars.demod_select.var_enum.BCR) and
                (modtype == model.vars.modulation_type.var_enum.FSK2 or
                 modtype == model.vars.modulation_type.var_enum.MSK)):
            ksi1 = int(round(127 * mi * gain))
        else:
            ksi1 = 0

        self._reg_sat_write(model.vars.MODEM_VITERBIDEMOD_VITERBIKSI1, ksi1, model.part_family)
        model.vars.ksi1_actual.value = model.vars.MODEM_VITERBIDEMOD_VITERBIKSI1.value
    ...
    def calc_ksi2_ksi3_reg(self, model):
        ...
        # ksi1 is alrady calculated based on PHSCALE
        ksi1 = model.vars.ksi1_actual.value
        ...
        self._reg_write(model.vars.MODEM_VITERBIDEMOD_VITERBIKSI2, int(best_ksi2))
        self._reg_write(model.vars.MODEM_VITERBIDEMOD_VITERBIKSI3, int(best_ksi3))
        self._reg_write(model.vars.MODEM_VTCORRCFG1_VITERBIKSI3WB, int(best_ksi3wb))
