...
class CALC_Synth_ocelot(CALC_Synth_lynx):
    ...
    def calc_rx_mode(self, model):
        ...
        if (etsi == model.vars.etsi_cat1_compatible.var_enum.Band_868) or \
            synth_settling_mode == model.vars.synth_settling_mode.var_enum.FAST:
    ...
    def calc_tx_mode(self, model):
        ...
        tx_power_dbm = model.vars.max_tx_power_dbm.value
        rf_freq = model.vars.base_frequency_hz.value

        if band == model.vars.rf_band.var_enum.BAND_2400:
            if baudrate > 1.5e6:
                model.vars.synth_tx_mode.value = model.vars.synth_tx_mode.var_enum.MODE5
            else:
                model.vars.synth_tx_mode.value = model.vars.synth_tx_mode.var_enum.MODE6
        elif baudrate > 250e3:
            model.vars.synth_tx_mode.value = model.vars.synth_tx_mode.var_enum.MODE1
        elif (tx_power_dbm > 20 or etsi == model.vars.etsi_cat1_compatible.var_enum.Band_868) and rf_freq > 510e6: # for bands < 510 MHz we can keep MODE2 to save power
            model.vars.synth_tx_mode.value = model.vars.synth_tx_mode.var_enum.MODE3
        else:
    ...
    def calc_rx_mode_reg(self, model):
        ...
        rx_mode_settings = [
            [1	,1	,0  ],      # SYNTH.LPFCTRL2RX.CASELRX.io
            [8	,16	,0  ],      # SYNTH.LPFCTRL2RX.CAVALRX.io
            [1	,1	,1  ],      # SYNTH.LPFCTRL2RX.CZSELRX.io
            [70	,229,254],      # SYNTH.LPFCTRL2RX.CZVALRX.io
            [0	,0	,0  ],      # SYNTH.LPFCTRL2RX.CFBSELRX.io
            [0	,0	,0  ],      # SYNTH.LPFCTRL2RX.LPFGNDSWENRX.io
            [2	,2	,2  ],      # SYNTH.LPFCTRL2RX.LPFINCAPRX.io
            [0	,0	,1  ],      # SYNTH.LPFCTRL2RX.MODESELRX.io
            [5	,0	,0  ],      # SYNTH.LPFCTRL1RX.OP1BWRX.io
            [13	,7	,8  ],      # SYNTH.LPFCTRL1RX.OP1COMPRX.io
            [0	,0	,2  ],      # SYNTH.LPFCTRL1RX.RFBVALRX.io
            [0	,7	,5  ],      # SYNTH.LPFCTRL1RX.RPVALRX.io
            [3	,13	,9  ],      # SYNTH.LPFCTRL1RX.RZVALRX.io
            [1	,1	,1  ],      # SYNTH.LPFCTRL2RX.LPFSWENRX.io
            [0	,0	,0  ],      # SYNTH.DSMCTRLRX.REQORDERRX.io
            [0	,1	,0  ],      # SYNTH.DSMCTRLRX.MASHORDERRX.io
            [1	,0	,1  ],      # SYNTH.DSMCTRLRX.DEMMODERX.io
            [0	,1	,0  ],      # SYNTH.DSMCTRLRX.LSBFORCERX.io
            [0	,1	,0  ],      # SYNTH.DSMCTRLRX.DSMMODERX.io
            [3	,0	,3  ],      # SYNTH.DSMCTRLRX.DITHERDACRX.io
            [7	,0	,7  ],      # SYNTH.DSMCTRLRX.DITHERDSMOUTPUTRX.io
            [1	,0	,1  ],      # SYNTH.DSMCTRLRX.DITHERDSMINPUTRX.i
            [2	,4	,2  ],      # RAC.SYMMDCTRL.SYMMDMODERX.io
            [4	,4	,4  ],      # RAC.SYTRIM1.SYLODIVLDOTRIMNDIORX.io
            [0	,0	,0  ],      # RAC.SYTRIM1.SYLODIVLDOTRIMCORERX.io
            [4	,4	,4  ],      # RAC.VCOCTRL.VCODETAMPLITUDERX.i
            [4	,5	,5  ],      # RAC.SYTRIM0.SYCHPCURRRX.io
            [1	,1	,1  ],      # RAC.SYTRIM0.SYCHPREPLICACURRADJ.
            [1	,0	,1  ],      # RAC.RX.SYCHPBIASTRIMBUFRX
            [0	,1	,0  ],      # RAC.RX.SYPFDCHPLPENRX
            [0	,1	,0  ],      # RAC.RX.SYPFDFPWENRX
            [0	,1	,0  ],      # RAC.SYEN.SYCHPLPENRX
            [7	,0	,7  ],      # RAC.SYTRIM0.SYCHPLEVPSRCRX
            [1	,0	,1  ]]      # RAC.SYTRIM0.SYCHPSRCENRX

        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_CASELRX          ,rx_mode_settings[0][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_CAVALRX          ,rx_mode_settings[1][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_CZSELRX          ,rx_mode_settings[2][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_CZVALRX          ,rx_mode_settings[3][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_CFBSELRX         ,rx_mode_settings[4][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_LPFGNDSWENRX     ,rx_mode_settings[5][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_LPFINCAPRX       ,rx_mode_settings[6][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_MODESELRX        ,rx_mode_settings[7][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL1RX_OP1BWRX          ,rx_mode_settings[8][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL1RX_OP1COMPRX        ,rx_mode_settings[9][ind] )
        self._reg_write(model.vars.SYNTH_LPFCTRL1RX_RFBVALRX         ,rx_mode_settings[10][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1RX_RPVALRX          ,rx_mode_settings[11][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1RX_RZVALRX          ,rx_mode_settings[12][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2RX_LPFSWENRX        ,rx_mode_settings[13][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_REQORDERRX        ,rx_mode_settings[14][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_MASHORDERRX       ,rx_mode_settings[15][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_DEMMODERX         ,rx_mode_settings[16][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_LSBFORCERX        ,rx_mode_settings[17][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_DSMMODERX         ,rx_mode_settings[18][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_DITHERDACRX       ,rx_mode_settings[19][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_DITHERDSMOUTPUTRX ,rx_mode_settings[20][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLRX_DITHERDSMINPUTRX  ,rx_mode_settings[21][ind])
        self._reg_write(model.vars.RAC_SYMMDCTRL_SYMMDMODERX         ,rx_mode_settings[22][ind])
        self._reg_write(model.vars.RAC_SYTRIM1_SYLODIVLDOTRIMNDIORX  ,rx_mode_settings[23][ind])
        self._reg_write(model.vars.RAC_SYTRIM1_SYLODIVLDOTRIMCORERX  ,rx_mode_settings[24][ind])
        self._reg_write(model.vars.RAC_VCOCTRL_VCODETAMPLITUDERX     ,rx_mode_settings[25][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPCURRRX           ,rx_mode_settings[26][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPREPLICACURRADJ   ,rx_mode_settings[27][ind])
        self._reg_write(model.vars.RAC_RX_SYCHPBIASTRIMBUFRX         ,rx_mode_settings[28][ind])
        self._reg_write(model.vars.RAC_RX_SYPFDCHPLPENRX             ,rx_mode_settings[29][ind])
        self._reg_write(model.vars.RAC_RX_SYPFDFPWENRX               ,rx_mode_settings[30][ind])
        self._reg_write(model.vars.RAC_SYEN_SYCHPLPENRX              ,rx_mode_settings[31][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPLEVPSRCRX        ,rx_mode_settings[32][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPSRCENRX          ,rx_mode_settings[33][ind])
    ...
    def calc_tx_mode_reg(self, model):
        ...
        tx_mode_settings = [
            [1	,1	,0	,1	,1	,1  ],      #SYNTH.LPFCTRL2TX.CASELTX.io
            [8	,16	,0	,15	,8	,15 ],      #SYNTH.LPFCTRL2TX.CAVALTX.io
            [1	,1	,1	,1	,1	,1  ],      #SYNTH.LPFCTRL2TX.CZSELTX.io
            [70	,229, 254, 126 ,70 ,126],      #SYNTH.LPFCTRL2TX.CZVALTX.io
            [0	,0	,0	,0	,0	,0  ],      #SYNTH.LPFCTRL2TX.CFBSELTX.io
            [0	,0	,0	,0	,0	,0  ],      #SYNTH.LPFCTRL2TX.LPFGNDSWENTX.io
            [2	,2	,2	,2	,2	,2  ],      #SYNTH.LPFCTRL2TX.LPFINCAPTX.io
            [0	,0	,1	,0	,0	,0  ],      #SYNTH.LPFCTRL2TX.MODESELTX.io
            [5	,0	,0	,0	,5	,0  ],      #SYNTH.LPFCTRL1TX.OP1BWTX.io
            [13	,7	,8	,13	,13	,13 ],      #SYNTH.LPFCTRL1TX.OP1COMPTX.io
            [0	,0	,2	,0	,0	,0  ],      #SYNTH.LPFCTR1TX.RFBVALTX.io
            [0	,7	,5	,0	,0	,0  ],      #SYNTH.LPFCTRL1TX.RPVALTX.io
            [3	,13	,9	,0	,3	,0  ],      #SYNTH.LPFCTRL1TX.RZVALTX.io
            [1	,1	,1	,1	,1	,1  ],      #SYNTH.LPFCTRL2TX.LPFSWENTX.i
            [4	,5	,5	,4	,5	,5  ],      #RAC.SYTRIM0.SYCHPCURRTX.io
            [0	,0	,0	,0	,0	,0  ],      #SYNTH.DSMCTRLTX.REQORDERTX.io
            [0	,1	,0	,0	,0	,0  ],      #SYNTH.DSMCTRLTX.MASHORDERTX.io
            [1	,0	,1	,1	,1	,1  ],      #SYNTH.DSMCTRLTX.DEMMODETX.io
            [0	,1	,0	,0	,0	,0  ],      #SYNTH.DSMCTRLTX.LSBFORCETX.io
            [0	,1	,0	,0	,0	,0  ],      #SYNTH.DSMCTRLTX.DSMMODETX.io
            [3	,0	,3	,3	,3	,3  ],      #SYNTH.DSMCTRLTX.DITHERDACTX.io
            [3	,0	,3	,3	,3	,3  ],      #SYNTH.DSMCTRLTX.DITHERDSMOUTPUTTX.io
            [1	,0	,1	,1	,1	,1  ],      #SYNTH.DSMCTRLTX.DITHERDSMINPUTTX.io
            [2	,4	,2	,2	,2	,2  ],      #RAC.SYMMDCTRL.SYMMDMODETX.io
            [4	,4	,4	,4	,4	,4  ],      #RAC.SYTRIM1.SYLODIVLDOTRIMNDIOTX.io
            [0	,0	,0	,0	,0	,0  ],      #RAC.SYTRIM1.SYLODIVLDOTRIMCORETX.io
            [4	,4	,4	,4	,4	,4  ],      #RAC.VCOCTRL.VCODETAMPLITUDETX.io
            [1	,0	,1	,1	,1	,1  ],      #RAC.TX.SYCHPBIASTRIMBUFTX
            [0	,1	,0	,0	,0	,0  ],      #RAC.TX.SYPFDCHPLPENTX
            [0	,1	,0	,0	,0	,0  ],      #RAC.TX.SYPFDFPWENTX
            [0	,1	,0	,0	,0	,0  ],      #RAC.SYEN.SYCHPLPENTX
            [7	,0	,7	,7	,7	,7  ],      #RAC.SYTRIM0.SYCHPLEVPSRCTX
            [1	,0	,1	,1	,1	,1  ]]      #RAC.SYTRIM0.SYCHPSRCENTX

        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_CASELTX         ,tx_mode_settings[ 0][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_CAVALTX         ,tx_mode_settings[ 1][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_CZSELTX         ,tx_mode_settings[ 2][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_CZVALTX         ,tx_mode_settings[ 3][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_CFBSELTX        ,tx_mode_settings[ 4][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_LPFGNDSWENTX    ,tx_mode_settings[ 5][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_LPFINCAPTX      ,tx_mode_settings[ 6][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_MODESELTX       ,tx_mode_settings[ 7][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1TX_OP1BWTX         ,tx_mode_settings[ 8][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1TX_OP1COMPTX       ,tx_mode_settings[ 9][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1TX_RFBVALTX         ,tx_mode_settings[10][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1TX_RPVALTX         ,tx_mode_settings[11][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL1TX_RZVALTX         ,tx_mode_settings[12][ind])
        self._reg_write(model.vars.SYNTH_LPFCTRL2TX_LPFSWENTX       ,tx_mode_settings[13][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPCURRTX          ,tx_mode_settings[14][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_REQORDERTX       ,tx_mode_settings[15][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_MASHORDERTX      ,tx_mode_settings[16][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_DEMMODETX        ,tx_mode_settings[17][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_LSBFORCETX       ,tx_mode_settings[18][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_DSMMODETX        ,tx_mode_settings[19][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_DITHERDACTX      ,tx_mode_settings[20][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_DITHERDSMOUTPUTTX,tx_mode_settings[21][ind])
        self._reg_write(model.vars.SYNTH_DSMCTRLTX_DITHERDSMINPUTTX ,tx_mode_settings[22][ind])
        self._reg_write(model.vars.RAC_SYMMDCTRL_SYMMDMODETX        ,tx_mode_settings[23][ind])
        self._reg_write(model.vars.RAC_SYTRIM1_SYLODIVLDOTRIMNDIOTX ,tx_mode_settings[24][ind])
        self._reg_write(model.vars.RAC_SYTRIM1_SYLODIVLDOTRIMCORETX ,tx_mode_settings[25][ind])
        self._reg_write(model.vars.RAC_VCOCTRL_VCODETAMPLITUDETX    ,tx_mode_settings[26][ind])
        self._reg_write(model.vars.RAC_TX_SYCHPBIASTRIMBUFTX        ,tx_mode_settings[27][ind])
        self._reg_write(model.vars.RAC_TX_SYPFDCHPLPENTX            ,tx_mode_settings[28][ind])
        self._reg_write(model.vars.RAC_TX_SYPFDFPWENTX              ,tx_mode_settings[29][ind])
        self._reg_write(model.vars.RAC_SYEN_SYCHPLPENTX             ,tx_mode_settings[30][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPLEVPSRCTX       ,tx_mode_settings[31][ind])
        self._reg_write(model.vars.RAC_SYTRIM0_SYCHPSRCENTX         ,tx_mode_settings[32][ind])
    ...
    def calc_synth_misc(self, model):
        ...
        self._reg_write(model.vars.RAC_SYNTHENCTRL_MMDPOWERBALANCEDISABLE, 1)
        self._reg_write(model.vars.RAC_SYTRIM0_SYTRIMCHPREGAMPBIAS, 0)
        self._reg_write(model.vars.RAC_SYTRIM0_SYTRIMCHPREGAMPBW, 3)
        self._reg_write(model.vars.RAC_SYTRIM1_SYTRIMMMDREGAMPBIAS, 0)
