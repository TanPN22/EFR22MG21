...
class CALC_Pro2_Demod_Ocelot(ICalculator):
    ...
    def buildVariables(self,model):
        ...
        self._addModelVariable(model, 'api_rsymb', int, ModelVariableFormat.DECIMAL)
    ...
    def calc_run_pro2(self,model):
        ...
        #bcr_cfe_costthd = model.vars.bcr_cfe_costthd.value
        ...
        api_rsymb = model.vars.api_rsymb.value
        ...
        #Create the input dict
            ...
            pro2_inputs["API_Rsymb"] = api_rsymb
    ...
    def _map_pro2_outputs(self,model, pro2_calculator_obj):
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODARR1_CFEQUALTHD, 2)
        self._reg_write(model.vars.MODEM_BCRDEMODARR1_SYMBWMAX, 8)
        self._reg_write(model.vars.MODEM_BCRDEMODARR1_SYMBWMIN, 2)
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODPMEXP_BCRCFESCHWIN , 3)
        self._reg_write(model.vars.MODEM_BCRDEMODPMEXP_BCRPMACQWIN  , 2)
    ...
    def write_unused_bcr_regs(self,model):
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODARR1_CFEQUALTHD, 0)
        ...
        self._reg_write(model.vars.MODEM_BCRDEMODPMEXP_BCRCFESCHWIN, 3)
        self._reg_write(model.vars.MODEM_BCRDEMODPMEXP_BCRPMACQWIN, 1)
    ...
    def calc_pro2_afc_mode(self,model):
        ...
        #     pro2_afc_mode = 1
        if (preamble_length >= 16) and ( mod_type!= 1) :
    ...
    def calc_pro2_fs_rx_ch(self,model):
        ...
        dec8 = 8
        manchester_en = (symbol_encoding == model.vars.symbol_encoding.var_enum.Manchester)

        pro2_fs_rx_ch = int(adc_freq_actual/dec8/dec0_actual/dec1_actual/(2**manchester_en))

        # Load local variables back into model variables
        model.vars.pro2_fs_rx_ch.value = pro2_fs_rx_ch


    def calc_bcr_demod_api_rsymb(self,model):
        #This function calculates api_rsymb based on the input bitrate and modulation/encoding settings
        ...
        #Based on modulation type calculate api_rsymb from bitrate
        ...
        #Adjust api_rsymb based on symbol encoding
        ...
        #Load local variables back into model variables
        model.vars.api_rsymb.value = int(round(api_rsymb))

    def calc_bcr_demod_ksi3(self,model):
        # This function calculates the KSI3 value for the BCR demod
        # Load model values into local variables
        ksi3_actual = model.vars.MODEM_VTCORRCFG1_VITERBIKSI3WB.value
        phscale_actual = model.vars.MODEM_TRECPMDET_PHSCALE.value
        ...
        bcr_calculated_ksi3_firstCalc = int(round(ksi3_actual / 2 ** phscale_actual))

        if (estosren_actual == 1):
            bcr_calculated_ksi3 = int(round(1.03 * bcr_calculated_ksi3_firstCalc * (freq_dev_min + freq_dev_max) / 2 / deviation))
        else:
            bcr_calculated_ksi3 = bcr_calculated_ksi3_firstCalc

        model.vars.calculated_ksi3.value = bcr_calculated_ksi3

    def calc_bcr_demod_phscale(self,model):
