...
def S37():
    ...
    if ("gcc" == "iar" or "gcc" == ""):
        ...
        GCC_TOOLCHAIN_PATH = shortPath(r"file:/D:/DH/Lumi/Zigbee/SimplicityStudio/SimplicityStudio/v4/developer/toolchains/gnu_arm/7.2_2017q4/".replace(
            "file:", "").replace("%20", " "))
...
def EBL():
    ...
    print (" ")
    EBL_COMMAND = START_FLAGS + shortPath(r"D:\DH\Lumi\Zigbee\SimplicityStudio\SimplicityStudio\v4\developer\adapter_packs\commander\commander.exe").replace(
...
def GBL():
    ...
    GBL_COMMAND = START_FLAGS + shortPath(r"D:\DH\Lumi\Zigbee\SimplicityStudio\SimplicityStudio\v4\developer\adapter_packs\commander\commander.exe").replace(
...
def OTA():
    if not '"%PROJECT_DIR%\..\\..\\v4\\developer\\sdks\\gecko_sdk_suite\\v2.7\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 0 --manuf-id 0x1002 --image-type 0 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL Z3LightSoc_FW1"':
        sys.exit(0)
    print (" ")
    ...
    IMAGE_BUILDER = '"%PROJECT_DIR%\..\\..\\v4\\developer\\sdks\\gecko_sdk_suite\\v2.7\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 0 --manuf-id 0x1002 --image-type 0 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL Z3LightSoc_FW1"'.replace("\t", "\\t")
